...
class Angel_Model_User extends Angel_Model_AbstractModel {
    ...
    public function addManageUser($email, $password, $salt, $checkemail = true) {
        $usertype = "admin";
        return $this->registerUser($email, $password, uniqid(), $usertype, $salt, $checkemail, NULL, NULL);
    }
    ...
    protected function registerUser($email, $password, $username, $usertype, $salt, $checkemail, $age, $gender) {
        $result = false;
        if (empty($email)) {
            throw new Angel_Exception_User(Angel_Exception_User::EMAIL_EMPTY);
        } else {
